...
public class FileValidator extends Validator{
    ...
    public ValidatorResponse validateFile(MultipartFile file){
        if (isNull(file)){
            return new ValidatorResponse(true, ValidatorMessages.OK);
        }

        if (file.getSize() > MAX_FILE_SIZE){
            return new ValidatorResponse(false, FIELD_NAME, ValidatorMessages.WRONG_FILE_SIZE_ERROR);
        }

        if (file.getOriginalFilename().length() > MAX_FILENAME_SIZE){
            return new ValidatorResponse(false, FIELD_NAME, ValidatorMessages.WRONG_FILENAME_SIZE_ERROR);
        }
